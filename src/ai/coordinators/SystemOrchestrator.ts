// Pour les types de feedback qui étaient en imports relatifs
import { FeedbackEntry, FeedbackAnalysis, FeedbackPattern, FeedbackRecommendation, QualityMetrics } from '@ai/feedback/types/feedback.types';
import { IEmotionFeedbackHandler } from '@ai/feedback/interfaces/IEmotionFeedbackHandler';

// Ces imports semblent déjà utiliser des alias corrects
...
// Pour les types d'émotion qui étaient en imports relatifs
import {
    EmotionAnalysis,
    EmotionAdjustments,
    EmotionPattern,
...
} from '@ai/feedback/types/emotion-feedback.types';
/**
...
export class EmotionFeedbackHandler implements IEmotionFeedbackHandler {
    ...
    private analyzeFacialComponents(components: EmotionState['components']) {
        return {
            eyebrows: components.facial.eyebrows,
            eyes: components.facial.eyes,
            mouth: components.facial.mouth,
            intensity: components.facial.intensity,
            quality: this.calculateFacialQuality(components.facial)
        };
    }
    ...
    private analyzeGesturalComponents(components: EmotionState['components']) {
        return {
            hands: components.gestural.hands,
            speed: components.gestural.speed,
            amplitude: components.gestural.amplitude,
            tension: components.gestural.tension,
            intensity: components.gestural.intensity,
            quality: this.calculateGesturalQuality(components.gestural)
        };
    }
    ...
    private convertExpectedImprovements(improvements?: ExpectedMetricImprovements): Partial<QualityMetrics> {
        ...
        Object.entries(improvements).forEach(([key, value]) => {
            if (!['accuracy', 'consistency', 'relevance', 'timeliness'].includes(key) && value !== undefined) {
                // Assurez-vous que value est un nombre
                if (value === null) {
                    additionalMetrics[key] = 0; // Valeur par défaut si null
                } else if (typeof value === 'object') {
                    // Si c'est un objet vide, convertir en nombre
                    additionalMetrics[key] = Object.keys(value).length;
                } else {
                    // Pour les autres cas, essayer de convertir en nombre
                    additionalMetrics[key] = Number(value) || 0;
                }
                hasAdditional = true;
            }
        });
