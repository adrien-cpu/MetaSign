...
export class TemporalGrammarValidator {
  ...
  private async extractMainTimeline(structure: GrammaticalStructure): Promise<TimelineSegment[]> {
    ...
    return structure.components
      .filter(comp => {
        const adapter = new ComponentAdapter(this.convertToGrammarComponent(comp));
  ...
  private buildSequence(component: unknown): Sequence {
    const adapter = new ComponentAdapter(this.convertToGrammarComponent(component));
    const elements = adapter.getElements();
  ...
  // Méthode utilitaire pour convertir n'importe quel composant en GrammarComponent
  private convertToGrammarComponent(component: unknown): GrammarComponent {
    if (!component || typeof component !== 'object') {
      return { id: '', type: '' };
    }

    // Si c'est un objet, on peut faire un cast de type sécurisé
    ...
    return {
      ...
      value: obj.value,
      ...obj // Copier toutes les autres propriétés
    };
  }
}
