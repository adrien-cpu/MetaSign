...
export class EnhancedAdaptationFactory {
    ...
    private static createDefaultAdvancedFeatures(): IAdvancedAdaptation {
        return {
            ...
            // Implémentation adaptée à l'interface EffectivenessEvaluationResult
            evaluateEffectiveness: async (adaptations, metrics) => {
                // Extraire des informations utiles des paramètres
                const adaptationId = Array.isArray(adaptations) && adaptations.length > 0 ?
                    `adaptation-${Date.now()}` : 'default-adaptation';

                // Utiliser metrics pour les scores
                const accuracyScore = metrics && typeof metrics === 'object' ?
                    (metrics.accuracy as number || 0.7) : 0.7;
                const responsivenessScore = metrics && typeof metrics === 'object' ?
                    (metrics.responsiveness as number || 0.8) : 0.8;

                // Créer un objet EffectivenessEvaluationResult conforme à l'interface
            ...
            // Implémentation adaptée à l'interface StrategyRefinementResult
            refineStrategy: async (evaluation, currentStrategies) => {
                // Utilisation des paramètres
                const strategyId = typeof evaluation === 'object' && evaluation !== null ?
                    'strategy-' + Object.keys(evaluation).length : 'default-strategy';

                // Récupérer les stratégies actuelles
                const strategies = Array.isArray(currentStrategies) ?
                    currentStrategies : ["default_strategy"];

                // Créer un objet StrategyRefinementResult conforme à l'interface
                const result: StrategyRefinementResult = {
                    ...
                    implementation: {
                        ...
                        parameters: {
                            ...
                            evaluationScore: typeof evaluation === 'object' &&
                                evaluation !== null &&
                                'score' in evaluation ?
    ...
    private static createDynamicSystem(
        ...
        return {
            ...
            evaluateEffectiveness: async (adaptations, metrics) => {
                ...
                // Utiliser metrics pour les scores si disponible
                const accuracyScore = metrics && typeof metrics === 'object' ?
                    (metrics.accuracy as number || 0.7) : 0.7;
                const responsivenessScore = metrics && typeof metrics === 'object' ?
                    (metrics.responsiveness as number || 0.8) : 0.8;
